...
| eSIM Auto Top-up v3 | ✅ | ❌ | ❌ | ❌ | Design lock: [esim-auto-topup-v3-design-lock.md](../design/esim-auto-topup-v3-design-lock.md); optional `active_until` lifetime guard (orthogonal to `renew_mode`); reason `schedule_ended`; does not replace `fixed_count`; does not amend ADR 010 |
