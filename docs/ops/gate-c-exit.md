...
| C7 | Sentry + uptime + reconcile alert | ⏳ **paused** | [observability.md](./releases/1.0.0/evidence/observability.md) — DSN/uptime/reconcile missing |
...
- [ ] Real `.env` (no `change-me`); `SENTRY_DSN` set — `.env` ✅; **`SENTRY_DSN` ⏳** (Stop Criteria)
...
**2026-07-25:** Host bootstrap + `/version` + billing E2E + dress-rehearsal smoke closed. **Paused on Observability (C7).**
